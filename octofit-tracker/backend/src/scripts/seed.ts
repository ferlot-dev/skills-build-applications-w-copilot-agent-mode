...
async function seedDatabase() {
  try {
    ...
    await Team.findByIdAndUpdate(teams[0]._id, { members: [users[1]._id, users[2]._id] });
    await Team.findByIdAndUpdate(teams[1]._id, { members: [users[0]._id, users[3]._id] });
    ...
    console.log(`Seeded teams: ${await Team.countDocuments()}`);
    console.log(`Seeded activities: ${await Activity.countDocuments()}`);
    console.log(`Seeded leaderboard entries: ${await Leaderboard.countDocuments()}`);
    console.log(`Seeded workouts: ${await Workout.countDocuments()}`);
